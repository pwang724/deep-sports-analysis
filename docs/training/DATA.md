...
| View ok | none exact. USO `shots.json` is method output, tuned and inspected, with uncertain spans marked `review` | none | Everything but the USO clip | Use USO keep / discard (not `review`) as provisional truth; spot-check disagreements by eye. |
| In play | Richard / Dylan cut lists: hand-reviewed, boundaries approximate | F3Set / E2E-Spot / TennisExpert rally clips | Other footage | Score Astra on times well inside or outside cut windows, not near boundaries. |
